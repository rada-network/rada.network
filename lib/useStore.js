...
const OpenBox = types
  .model({
    isOpening: types.optional(types.boolean, false),
    numberBox: types.optional(types.integer, 1),
    tokenIds: types.optional(types.array(TokenID), []),
    isBoxOpening: types.optional(types.boolean, false),
    boxOpened: types.optional(types.boolean, false),
    openSuccessfullyBoxs: types.optional(types.array(TokenID), []),
    openedNumberBox: types.optional(types.integer, 0),
    isError: types.optional(types.boolean, false),
    message: types.optional(types.string, ""),
  })
  .views((self) => ({}))
  .actions((self) => ({
    showOpenBoxModal(isOpen, numberBox) {
      self.isOpening = isOpen;
      self.numberBox = numberBox;
    },
    updateOpenBoxSuccessfully(boxs) {
      self.openSuccessfullyBoxs.push(boxs);
      self.openedNumberBox  = self.openSuccessfullyBoxs.length;
    },
    ...
    updateOpenBox(isOpened) {
      self.boxOpened = isOpened;
    },
    reset() {
      self.tokenIds = [];
      ...
      self.isError = false;
      self.message = "";
      self.openedNumberBox = 0;
      self.openSuccessfullyBoxs = [];
    },
    updateError(message, isError) {
      self.isError = isError;
      self.message = message;
