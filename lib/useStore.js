...
export function initializeStore() {
  ...
  if (typeof window !== 'undefined'){
    const localStorageNetwork = window.localStorage.getItem(NetworkLocalStorageKey)
    if (['bsc', 'eth'].indexOf(network) !== -1){
      network = localStorageNetwork
    }
  }
  const _store = store ?? Store.create({wallet: {},user : {},network : network})
