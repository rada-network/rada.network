...
const State = types.model({
  social_order: types.optional(types.string, 'popular'),
}).views(self => ({
  get socialOrder() {
    return self.social_order ?? 'popular'
  }
})).actions(self => ({
...
const StateIdeas = types.model({
  ideasOrder: types.optional(types.string, 'latest'),
}).views(self => ({
  get ideasOrder_() {
    // console.log("get ideasOrder: ", self.ideasOrder)
    return self.ideasOrder ?? "latest"
  }
  })).actions(self => ({
  setIdeasOrder(order){
    self.ideasOrder = order
    // console.log("setIdeasOrder: ", self.ideasOrder)
  }
}))
...
const Store = types
  .model({
    ...
    stateIdeas: types.optional(StateIdeas, {}),
    inited: false
  }).actions((self) => ({
    afterCreate() {
