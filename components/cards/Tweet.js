...
export const TweetCard = ({post, favoriteCount, retweetCount, source, hashtags, fullText, createdAt}) => {
  ...
  // console.log(uniqueHashtag.includes(""))
  const showPlatform = (hashtags) => {
    ...
    return (
      uniqueHashtag.map((hashtag) => (
        <a href={"https://cadano.org"} className={`metadata project-metadata_platform project-metadata_platform_${
          hashtag || ""} `}
        >
          <span className="icon">
            <i className={`cf cf-${hashtag.slice(0, 3).toLowerCase()} || "btc"`}></i>
              </span>
          <span className="metadata-value">{hashtag !== "" ? hashtag : "unavailable"}</span>
          <div className="metadata-divider"></div>
        </a>
      ))
    )
  }
  // https://twitter.com/user_id_str/status/id_twitter
  //
  return(
      ...
      <div className="card-body">
        <div className="card-body-header">
          <div className="card-title">
            <Link href={`/item/${post.id}`}>
              <a className="card-link">{fullText.slice(0, 50)}...</a>
            </Link>
            <a
              href={`/explore/"dapp"`}
        ...
        <div className="card-body-main">
          <div className="card-text" dangerouslySetInnerHTML={{ __html: fullText.slice(0, 30)}}></div>
        </div>
        ...
        <div className="card-body-footer">
          <div className="metadata-wrapper project-metadata-wrapper">
            ...
            <a href={"#"} className="metadata project-comment_count">
