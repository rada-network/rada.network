  ...
  return (
    <div className="flex flex-wrap justify-between items-center w-full">
      <div className="flex flex-0 flex-shrink-0 mb-4 items-center">
        <span className="icon flex-shrink-0 mr-2 h-px-32 w-px-32">
          {token && (
            <Image
              src={
                token?.logo !== null
        ...
        <h1 className="flex items-center">
          <strong className="text-xl lg:text-2xl font-semibold">
            {tokenData?.name}
          </strong>
      ...
      <div className="flex flex-wrap space-x-2 mb-4">
        {tokenData?.tag?.map((item) => (
          <span
            key={item.id}
            className={`badge badge-lg ${
              trendingStore.data.find((t) => t === item.slug) ? "badge-red" : ""
            }`}
          >
            {item.name}
          </span>
